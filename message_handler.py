...
def messageHandler(update: Update, context: CallbackContext):
    ...
    if Basket in update.message.text:
        ...
        context.bot.send_message(chat_id=update.effective_chat.id,
                                 reply_markup=InlineKeyboardMarkup(buttons), text=text2)
    ...
    if Order_Status in update.message.text:
        ...
        cursor = connection.cursor()
        cursor.execute(
            f'select top 1 status  from bot2 where phone_number=? order by time desc ;', contac.phone_number2)
        context.bot.send_message(
            chat_id=update.effective_chat.id, text=cursor.fetchone()[0])
        connection.commit()
